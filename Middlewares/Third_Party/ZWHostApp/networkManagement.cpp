...
void (__cdecl *cbNetworkManagement_ZW_SendData)(BYTE, WORD wTime);
/* Internal callback funtion used for sampling transmit time for NH/Maintenance ZW_SendData usage. */
void (__cdecl *cbNetworkManagement_Timed_ZW_SendData)(BYTE, WORD wTime);
/* */
	...
	WORD wTime)
{
    ...
    if (NULL != spCurrentSample)
    {
		...
		if (wTime > 0)
		{
			...
			spCurrentSample->sampleTime = wTime * 10;
		}
		else
		{
    ...
    {
        cbNetworkManagement_Timed_ZW_SendData(bTxStatus, wTime);
    }
}
    ...
    void (__cdecl *completedFunc)(BYTE, WORD))
{
    ...
    if ((0 != retVal) && (NULL != spCurrentSample))
    {
		/* HOST transmit metrics sampling specifics - Only needed if hw do not support transmit latency sampling */
	...
	WORD wTime)
{
    ...
    {
        cbNetworkManagement_ZW_SendData(bTxStatus, wTime);
    }
}
...
NetworkManagement_ZW_SendData(
    ...
    BYTE *pData,
    BYTE bDataLength,
    void (__cdecl *completedFunc)(BYTE, WORD wTime))
{
    bool retVal;
...
NetworkManagement_NH_TestConnection(
    BYTE bNodeID,
    void (*CompletedFunc)(BYTE bStatus, WORD wTime))
{
