...
// Rece?ed Z-wave Received frame is singlecast frame - (status == xxxx00xx)
#define RECEIVE_STATUS_TYPE_SINGLE  0x00
// Rece?ed Z-wave Received frame is broadcast frame - (status == xxxx01xx)
#define RECEIVE_STATUS_TYPE_BROAD   0x04
// Rece?ed Z-wave Received frame is multicast frame - (status == xxxx10xx)
...
#define ZW_MAX_NODES                    16  //232
#define ZW_MAX_REPEATERS                2   //4
#define ZW_MAX_NODEMASK_LENGTH          (ZW_MAX_NODES/8)

#define ZW_MAX_NODES_IN_MULTICAST       8	//64     /* Maximum number of nodes in a multicast */
...
public:
    ...
    BYTE ZW_SendData(BYTE nodeID, BYTE *pData, BYTE dataLength, BYTE txOptions, void (*completedFunc)(BYTE txStatus, WORD wTime));
...
void debuglog_wr_frame(const char *str, unsigned char *buf, BYTE buflen);
