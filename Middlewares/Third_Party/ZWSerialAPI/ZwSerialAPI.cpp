...
extern	osSemaphoreId ZwCommandQueueHandle;
extern	osSemaphoreId ZwTransmitQueueHandle;


UART_HandleTypeDef	*ZWUart = 0;

/* Buffer size */
#define BUF_SIZE    128	//256 /* enough */
...
/* ZwCommand queue */
struct ZwCommandElement
{
  BYTE  buf[BUF_SIZE];      /* xxxx       */
  BYTE  length;         /* Frame type */
} ZwCommandQueue[QUEUE_MAX];
static BYTE ZwQueueIndex = 0;
...
static struct _ZwTransmit_element_
{
  BYTE      wLen;
  BYTE      wBuf[BUF_SIZE];
  BYTE      *transmitStatus;
} ZwTransmitQueue[MAX_TX_QUEUE];

static BYTE ZwTransmitIndex = 0;
...
static void (*cbFuncZWSendData)(BYTE txStatus, WORD wTime);
    ...
    BYTE buflen)
{
    ...
    {
        snprintf(&framestr[n], 175*3-n, "%02X ", buf[i]);
        n += 3;
    }
    debuglog_wr("%s%s[L=%d]\r\n", str, framestr, buflen);
    delete framestr;
    ...
    BOOL *bStop)
{
    ...
    while ( ch != SOF )
    {
        ...
        {
            *len = 0;
            debuglog_wr("Received - %s [%01u]\r\n", (ch == ACK) ? "ACK" : "NAK", waitForAck);
            if (waitForAck)
            {
        ...
        else
        {
            if (ch == CAN)
            {
                ...
                debuglog_wr("Received - CAN [%01u]\r\n", waitForAck);
                if (waitForAck)
                {
                    if (canRetry)
                    {
                        /* We have been CANned */
                        CSerial::serPutChar(h, ACK);   // Tell the world we are happy...
                        debuglog_wr("Transmit - ACK CAN [%02u]\r\n", canRetry);
                    }
            ...
            {
                debuglog_wr("Received - Garbage %02X\r\n", ch);
            }
        }
        if (*bStop)
        {
...
//        printf("D3[%02X]\r\n", ch);
    ...
    if (CalculateChecksum(buffer, i-1) != buffer[i - 1])
    {
        ...
        CSerial::serPutChar(h, NAK);   // Tell the sender something went wrong
        debuglog_wr("Transmit - NAK\r\n");
        return statusFrameErr;
    }
    buffer[i - 1]=0;
    if (0 == CSerial::serPutChar(h, ACK))
    {
        debuglog_wr("Transmit - ACK retry\r\n");
        CSerial::serPutChar(h, ACK);
    }
    *len = i - 1;
    ...
    BYTE length)
{
    ...
    CSerial::serPutChar(h, SOF);
    CSerial::serWrite(h, wBuf, wLen);
    CSerial::serPutChar(h, byChk);
    waitForAck = TRUE;
    ...
    BYTE *bStatus)
{
	...
	osSemaphoreWait(ZwTransmitQueueHandle, 0);
    if (ZwTransmitIndex < MAX_TX_QUEUE)
    {
        ZwTransmitQueue[ZwTransmitIndex].transmitStatus = bStatus;
        ZwTransmitQueue[ZwTransmitIndex].wLen = length;
        memcpy(&ZwTransmitQueue[ZwTransmitIndex].wBuf[0], buffer, length);
        ZwTransmitIndex++;
    }
	osSemaphoreRelease(ZwTransmitQueueHandle);
}
...
//    printf("App: <-- Enter WaitForResponse\r\n");
    // Wait until response is received or the transmit thread signal us to stop
    // because all of the transmit retries timed out after 2 minuttes.
    timeOutVal = 5000;	//SetTimeOut(500); /* We wait max. 5 sec for the report */
    while (waitForResponse && !TimeOut)
    {
        while ((RecvBuffer[0] == 0) && waitForResponse)
        {
            ...
            if (TimeOut)
            {
//                printf("App: <-- WaitForResponse: timeOut\r\n");
                break;
            }
            osDelay(1);
        }
//        printf("App: <--RecvBuffer[%d] \r\n", RecvBuffer[0]);
        if (!TimeOut && waitForResponse && (RecvBuffer[1] == RESPONSE) && (RecvBuffer[2] == byFunc))
        {
            bFound = TRUE;
            waitForResponse = FALSE;
//            printf("App: <-- WaitForResponse: Found\r\n");
            break;
        }
    }
    waitForResponse = FALSE;
    ...
    RecvBuffer[0] = 0;
//    printf("App: <-- Exit WaitForResponse(TO=%d)\r\n",timeOutVal);
}
...
void DispatchProcTask(void const *argument)
{
//    CSerialAPI* pObject = (CSerialAPI*)pParam;
    ...
    osDelay(100);
    printf("\r\nZW_DispatchTask start\r\n");

   while (1)	//(!pObject->m_bEndThread)
    {
        if (ZwQueueIndex > 0)
        {
        	osSemaphoreWait(ZwCommandQueueHandle, 0);

            ZwQueueIndex--;
            length = ZwCommandQueue[0].length;

            memset(buf, 0, sizeof(buf));
            memcpy(buf,ZwCommandQueue[0].buf,ZwCommandQueue[0].length);
            /* POP the message from the queue (fifo) */
            memcpy((BYTE *)&ZwCommandQueue[0], (BYTE *)&ZwCommandQueue[1],
                    ZwQueueIndex * sizeof(struct ZwCommandElement));
        	osSemaphoreRelease(ZwCommandQueueHandle);
            api.Dispatch(buf,length);
        }
        else
        {
            osDelay(20);
...
void APIProcTask(void const *argument)
{
    unsigned char buf[BUF_SIZE];  //[4100];
    BYTE length;
    BYTE status;
    BOOL	bStop = FALSE;
    ...
    for (;;)
    {
        ...
        if ((ZwTransmitIndex > 0) && !waitForAck)
        {
            if (ZwTransmitQueue[0].transmitStatus != NULL)
            {
                *(ZwTransmitQueue[0].transmitStatus) = FRAME_TRANSMITTING;
            }
            api.TransmitData(ZwTransmitQueue[0].wBuf, ZwTransmitQueue[0].wLen);
            // Have set waitforack
        }

        length = 0;
        status = api.ReceiveData(buf, &length, &bStop);
        switch (status)
        {
            case statusFrameReceived:
                {
                    ...
                    if (length > 0)
                    {
                        if (buf[1] == REQUEST)
                        {
                        	...
                        	osSemaphoreWait(ZwCommandQueueHandle,0);
                            if (ZwQueueIndex < QUEUE_MAX)
                            {
                                memcpy(ZwCommandQueue[ZwQueueIndex].buf,buf,length);
                                ZwCommandQueue[ZwQueueIndex].length = length;
                                ZwQueueIndex++;
                   ...
                   osDelay(1);   /* Release CPU, so we do not use all the resources! */
                }
                break;
            ...
            case statusFrameSent:
                ...
                if (waitForAck)
                {
                	osSemaphoreWait(ZwTransmitQueueHandle,0);
                    ZwTransmitIndex--;
                    memcpy((BYTE *)&ZwTransmitQueue[0], (BYTE *)&ZwTransmitQueue[1],
                           ZwTransmitIndex * sizeof(struct _ZwTransmit_element_));
                	osSemaphoreRelease(ZwTransmitQueueHandle);
                    waitForAck = FALSE;
                    retry = 0;
                    canRetry = 0;
            ...
            case statusTxCANned:
                ...
                else if ((api.learnState) && (api.TimeOut))
                {
                    waitForAck = FALSE;
                    waitForResponse = FALSE;
                    api.CommLost(CSerialAPI::COMM_NO_RESPONSE);
                }
                break;
...
//                ASSERT(funcApplicationCommandHandler_Bridge != NULL);
        ...
        case FUNC_ID_ZW_SEND_DATA:
            if (cbFuncZWSendData != NULL)
            {
                if (bylen > 5)
                {
                    cbFuncZWSendData(pData[IDX_DATA + 1], pData[IDX_DATA + 2] * 256 + pData[IDX_DATA + 3]);
                }
                else
                {
                    cbFuncZWSendData(pData[IDX_DATA + 1], 0);
                }
            }
            break;
  ...
  void(*funcCommError)(BYTE))
{
   ...
   printf("CSerialAPI(%d)::Initialize\r\n", byPort);
...
//    ASSERT(funcCommError != NULL);
 ...
 //       m_hComPort = CSerial::serOpen(pComPort, bySpeed);
    	...
    	if (ZWUart)
        {
            ...
            CSerial::serPutChar(ZWUart, ACK);   // Tell the world we are happy...
//    		printf("CSerialAPI::ZWUart send ACK\r\n");
        }
    }
    return (ZWUart != NULL);
}
...
BYTE CSerialAPI::ZW_SendData(
    ...
    BYTE dataLength, 
    BYTE txOptions, 
    void (__cdecl *completedFunc)(BYTE, WORD))
{
    int i;
    BYTE idx=0;
    BYTE byCompletedFunc = (completedFunc == NULL? 0: SeqNo());
